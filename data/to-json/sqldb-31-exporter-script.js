...
function makeWordNet(input_path, preload){
  var db;
	...
	if (wn.preload === true){
		...
		ret = wn.SEMLINKS.then(function(d){
			...
			fs.writeFileSync('d:\\home\\semlinks.json', data);
			return res;
		});

		ret = wn.SAMPLES.then(function(d){
			// var search_lemma = inputs[0];
			// var res;
			// var candidateSet = d[search_lemma];
			// for (var i = 0; i < candidateSet.length; i++)
			...
			// }
			let data = JSON.stringify(d);
			fs.writeFileSync('d:\\home\\samples.json', data);
			return res;
		});
      ...
      "FROM wordsXsensesXsynsets AS s LEFT JOIN lexdomains AS l ON l.lexdomainid = s.lexdomainid";
		...
		ret = wn.SEMLINKS.SISTERS.then(function(d){
			// var search_lemma = inputs[0];
			// var res;
			// var candidateSet = d[search_lemma];
			// for (var i = 0; i < candidateSet.length; i++)
			...
			// }
			let data = JSON.stringify(d);
			fs.writeFileSync('d:\\home\\semlinks.sisters.json', data);
			return res;
		});
		...
		ret = wn.SYNSETSxLEXDOMAINS.then(function(d){
			// var search_lemma = inputs[0];
			// var res;
			// var candidateSet = d[search_lemma];
			// for (var i = 0; i < candidateSet.length; i++)
			...
			// }
			let data = JSON.stringify(d);
			fs.writeFileSync('d:\\home\\synsetsxlexdomains.json', data);
			return res;
		});
	}
	...
	wn.morphyPromise = function(input_str, pos){
		...
		if (!wn.DICTIONARY){
		  ...
		  wn.DICTIONARY = db.allAsync(query).map(function(elem){
			...
			wn.DICTIONARY.then(function(d){
				...
				// }
				let data = JSON.stringify(d);
				fs.writeFileSync('d:\\home\\'+"DICTIONARY".toLowerCase()+'.json', data);
		...
		if (!wn.EXCEPTIONS){
		  ...
		  wn.EXCEPTIONS.then(function(d){
			...
			// }
			let data = JSON.stringify(d);
			fs.writeFileSync('d:\\home\\'+"EXCEPTIONS".toLowerCase()+'.json', data);
			return res;
		});
		}
	...
	wn.Word.prototype = {
				...
				if (!wn.preload){
				  ...
				  ret =  db.allAsync(query,{
					...
					ret =  new Promise(function(resolve, reject) {
						...
						var d=require("d:/home/synset.json")
	  ...
	  getAntonyms: function(callback){
		...
		function _formAntonymsArray(synsetid){
		  ...
		  query += "WHERE  linkid=30 AND spos='a' ORDER BY ssensenum";
		 //return db.allAsync(query);


		  tttt =  db.allAsync(query);				 

		  ret =  	tttt.then(function(d){

			 var mmm= _.groupBy(d, "antonym");
			 let ffff = JSON.stringify(mmm);
			 fs.writeFileSync('d:\\home\\'+"_formAntonymsArray".toLowerCase()+'.json', ffff);
			 var eeeeee= mmm[word];
		...
		function _findWordId(str){
			var tttt= db.allAsync("SELECT wordid,lemma FROM words");
			return	tttt.then(function(d){
			  
			  var mmm= _.groupBy(d, "lemma");
			  let data = JSON.stringify(mmm);
			  fs.writeFileSync('d:\\home\\'+"_findWordId".toLowerCase()+'.json', data);
			  var eeeeee= mmm[str];
      ...
      var query, ret;
			switch(self.part_of_speech){
			case null:
				query = "SELECT synsetid,wordid FROM senses";
				var tttt=db.allAsync(query);

					 ret =  	tttt.then(function(d){
			  
						var mmm= _.groupBy(d, "wordid");
						let ffff = JSON.stringify(mmm);
						fs.writeFileSync('d:\\home\\'+"_findSynsetsFromId_all_parts_of_speech".toLowerCase()+'.json', ffff);
						var eeeeee= mmm[data.wordid];
			
						return eeeeee.map(function(elem){
							return elem.synsetid;
						});
					});
			break;
			default:
			    query = "SELECT senses.synsetid,wordid,pos FROM senses INNER JOIN synsets ON synsets.synsetid = senses.synsetid";
			   ...
			   var  tttt =  db.allAsync(query);				 
				 ...
				 ret =  	tttt.then(function(d){
					...
					var mmm= _.groupBy(d, "wordid");
					let ffff = JSON.stringify(mmm);
					fs.writeFileSync('d:\\home\\'+"_findSynsetsFromId_one_parts_of_speech".toLowerCase()+'.json', ffff);
					var eeeeee= _.groupBy( mmm[data.wordid], "pos")[self.part_of_speech];
		 ...
		 // query += "WHERE s.pos = $pos AND s.lemma = $lemma AND s.sensenum = $sensenum";
	  	...
	  	ret = wn.WORDSxSENSESxSYNSETSxLEXDOMAINS.then(function(d){
	  		...
	  		for (var i = 0; i < candidateSet.length; i++)
	  		{
			  	...
			  	//    ret.then(function(m,n){

		// 	 let data = JSON.stringify(m);
        //    fs.writeFileSync('d:\\home\\synset.json', data);
		//    });
	  		return res;
	  	});
	  }
	...
	function _findLemmasArray(synsetid){
		var query = "SELECT words.lemma,senses.synsetid FROM words INNER JOIN senses ON words.wordid = senses.wordid";
		var tttt= db.allAsync(query);
	...
	return	tttt.then(function(d){
			...
			// 	}
			// }
			var mmm= _.groupBy(d, "synsetid");
			let data = JSON.stringify(mmm);
			fs.writeFileSync('d:\\home\\'+"_findLemmasArray".toLowerCase()+'.json', data);
			var eeeeee= mmm[synsetid];
